...
class UptimeKumaSocketService: NSObject, ObservableObject {
    ...
    // KORRIGIERTE parseMonitors-Methode
    private func parseMonitors(_ data: [[String: Any]]) {
        var parsedMonitors: [Monitor] = []
        
        for item in data {
            // Konvertiere lastCheck von Int64 zu Date
            let lastCheckDate: Date?
            if let lastCheckMillis = item["lastCheck"] as? Int64 {
                lastCheckDate = Date(timeIntervalSince1970: TimeInterval(lastCheckMillis) / 1000)
            } else {
                lastCheckDate = nil
            }
